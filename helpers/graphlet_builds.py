...
def count_graphlets_between_nodes(graph, source, target):
    ...
    four_chordal_cycle_count = sum(1 for cycle in nx.simple_cycles(graph) if len(cycle) == 4 and source in cycle and target in cycle)
    four_tailed_triangle_count = sum(1 for _ in nx.all_simple_paths(graph, source=source, target=target, cutoff=3) if len(set(_)) == 4 and _[-1] == target)
    four_cycle_count = sum(1 for cycle in nx.simple_cycles(graph) if len(cycle) == 4 and source in cycle and target in cycle)
    ...
    return [triangle_count, triangle_count, two_star_count, four_clique_count, four_chordal_cycle_count,
            four_tailed_triangle_count, four_cycle_count, three_star_count, four_path_count, all_shortest_paths]
...
def get_graphlet_counts(G_given):
    ...
    columns = ['src', 'dst', 'triangle', 'triangle', '2-star', '4-clique', '4-chordal-cycle', '4-tailed-triangle', '4-cycle', '3-star', '4-path', "all_shortest_paths"]
