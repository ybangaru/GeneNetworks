...
def simulate_data(n_cells=1000):
    ...
    cell_types = {"CELL_ID": cell_ids, "CELL_TYPE": ["TYPE%d" % np.random.randint(0, 10) for _ in range(n_cells)]}
    cell_types = pd.DataFrame(cell_types)

    cell_bm_exp = {"CELL_ID": cell_ids}
    for i in range(40):
        cell_bm_exp["BIOMARKER%d" % i] = np.random.uniform(0.0, 100.0, (n_cells,))
...
def build_dataset(n_regions=5, region_prefix="REGION"):
    ...
    feature_kwargs = {
        ...
        "biomarker_expression_lower_bound": 0.0,
        "biomarker_expression_upper_bound": 100.0,
...
def test_construct_graph_for_region():
    ...
    assert len(G) == n_cells
    for n in G.nodes:
        assert G.nodes[n].keys() == set(
            ["voronoi_polygon", "cell_id", "center_coord", "cell_type", "biomarker_expression", "FEAT1", "FEAT2"]
        )
...
def test_construct_graph_with_incomplete_features():
    ...
    G = construct_graph_for_region(
        "region", os.path.join(root, "cell_coords.csv"), cell_types_file=None, cell_biomarker_expression_file=None
    )
    assert len(G) == n_cells
    for n in G.nodes:
...
def test_subgraph_sampler():
    ...
    sampler = SubgraphSampler(dataset, selected_inds=selected_inds, num_regions_per_segment=0)
    for _ in range(5):
        batch = next(sampler)
    ...
    sampler = SubgraphSampler(dataset, selected_inds=subset_regions, num_regions_per_segment=0)
    for _ in range(5):
        batch = next(sampler)
        assert len(set(batch.region_id) - set("%s%d" % (prefix, i) for i in selected_inds)) == 0

    dataset.save_all_subgraphs_to_chunk()
    sampler = SubgraphSampler(dataset, selected_inds=subset_regions, num_regions_per_segment=2, steps_per_segment=3)
    for _ in range(5):
        batch = next(sampler)
        assert len(set(batch.region_id) - set("%s%d" % (prefix, i) for i in selected_inds)) == 0
...
def test_get_random_sampled_subgraphs():
    ...
    dataset = build_dataset(n_regions=n_regions, region_prefix=prefix)
    selected_inds = np.random.choice(np.arange(n_regions), 5, replace=False)
    data_list = get_random_sampled_subgraphs(dataset, inds=selected_inds, n_samples=129, batch_size=32)
    assert all([data.region_id in ["%s%d" % (prefix, i) for i in selected_inds] for data in data_list])

    selected_inds = np.random.choice(np.arange(n_regions), 5, replace=False)
    subset_regions = ["%s%d" % (prefix, i) for i in selected_inds]
    data_list = get_random_sampled_subgraphs(dataset, inds=subset_regions, n_samples=129, batch_size=32)
    assert all([data.region_id in ["%s%d" % (prefix, i) for i in selected_inds] for data in data_list])
...
def test_model_overfit():
    ...
    train_kwargs = {
        ...
        "evaluate_fn": [
            helpers.train.evaluate_by_sampling_subgraphs,
            helpers.train.evaluate_by_full_graph,
            helpers.train.save_model_weight,
        ],
        "evaluate_freq": 10,
    ...
    model = helpers.train.train_subgraph(
        model, dataset, device, train_inds=[0, 1, 2, 3], valid_inds=None, **train_kwargs
    )
    ...
    _, graph_preds = helpers.inference.collect_predict_for_all_nodes(
        model, dataset, device, inds=np.arange(4), print_progress=False
    )
    res = helpers.inference.full_graph_graph_classification_evaluate_fn(dataset, graph_preds)
    assert res[0] == 1.0
