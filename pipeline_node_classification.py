...
import helpers
from helpers import CellularGraphDataset, train_subgraph
from sklearn.model_selection import train_test_split
import numpy as np

from helpers import SLICE_PIXELS_EDGE_CUTOFF, logger, NO_JOBS
...
def train_node_classification(dataset_kwargs):
    ...
    if dataset_kwargs["node_features_mask"]:
        use_node_features = [x for x in dataset_kwargs["node_features"] if x not in dataset_kwargs["node_features_mask"]]
        transformers.append(helpers.FeatureMask(dataset, use_center_node_features=use_node_features, use_neighbor_node_features=use_node_features, use_edge_features=dataset.edge_feature_names))
    ...
    # model_file = "/data/qd452774/spatial_transcriptomics/data/Liver12Slice12_leiden_res_0.7/model_Liver12Slice12_0.7/voronoi_delaunay-gin-emb_size_128-batch_size=128-lr=0.01/model_save_3188.pt"
...
def build_train_kwargs(network_type, graph_type, kwargs):
    ...
    NODE_FEATURES = ["cell_type", "center_coord", "biomarker_expression", "volume", 
                     "neighborhood_composition", 'perimeter', 'area', 'mean_radius', 
                     'compactness_circularity', 'fractality', 'concavity', 
                     'elongation', 'overlap_index', 'sbro_orientation', 'wswo_orientation', 
